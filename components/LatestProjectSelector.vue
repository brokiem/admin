      ...
      <div class="relative mt-1">
        ...
        <transition leave-active-class="transition ease-in duration-100" leave-from-class="opacity-100" leave-to-class="opacity-0">
          <ListboxOptions class="absolute z-10 mt-1 max-h-56 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
            <ListboxOption as="template" v-for="(project, title) in projects" :value="title" v-slot="{ active, selected }">
              <li :class="[active ? 'text-white bg-indigo-600' : 'text-gray-900', 'relative cursor-default select-none py-2 pl-3 pr-9']">
                <div class="flex items-center">
                  ...
                  <span :class="[selected ? 'font-semibold' : 'font-normal', 'ml-3 block truncate']">{{ title }}</span>
                </div>
...
function searchArrayByTitle(title, array) {
...
let title = useState('title', () => Object.keys(latestProject.value)[0]);
const selected = useState('selected', () => searchArrayByTitle(title.value, projects.value))

watch(title, (newTitle) => {
  const newSelected = searchArrayByTitle(newTitle, projects.value)
  selected.value = newSelected

  latestProject.value = {
    [newTitle]: newSelected
  }
