    ...
    <div class="min-h-full">
      <header class="bg-white shadow">
        <div class="flex flex-row mx-auto max-w-7xl py-6 px-4 sm:px-6 lg:px-8 gap-4">
          ...
          <button @click="refresh" type="button" class="items-center rounded-md border border-indigo-600 px-2 py-2 text-sm font-medium text-indigo-600 shadow-sm hover:bg-indigo-700 hover:text-white focus:outline-none focus:ring-indigo-500 focus:ring-0 focus:ring-offset-2 transition duration-300">
...
const { pending, data: lanyard, refresh } = useFetch(() => 'https://api.lanyard.rest/v1/users/548120702373593090')
...
watch(lanyard, (newLanyard) => {
  projects.value = JSON.parse(newLanyard.data.kv.projects)

  latestProject.value = JSON.parse(newLanyard.data.kv.latest_project)

  anime.value = JSON.parse(newLanyard.data.kv.fav_anime)
})
